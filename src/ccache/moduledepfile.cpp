...
template<> struct from<JSON, modules::depfiles::path_view>
{
  ...
  {
    read<JSON>::op<Opts>(value.repr, args...);
  }
};
...
path_view::path_view(std::string_view view) noexcept : repr(view)
{
}
...
{
  return std::filesystem::path(repr);
}

DepFile::ParseError::ParseError(glz::error_ctx&& ctx) : repr(ctx)
{
}

DepFile::ParseError::operator bool() const noexcept
{
  return repr.operator bool();
}
...
DepFile::ParseError::format(std::string_view buffer) const noexcept
  -> std::string
{
  return glz::format_error(repr, buffer);
}
  ...
  -> tl::expected<DepFile, DepFile::ParseError>
{
  ...
  if (err) {
    return tl::unexpected(DepFile::ParseError(std::move(err)));
  }
  return dep_file;
}
} // namespace modules::depfiles
