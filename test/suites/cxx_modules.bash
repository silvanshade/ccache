...
cxx_modules_flag_gcc() {
    CXX_MODULES_PROBE_GCC=(-std=c++20 -fdeps-format=p1689r5 -fdeps-file=probe.ddi -MD -MF probe.d -o probe.o -x c++ -c -)
...
SUITE_cxx_modules() {
    ...
    if $COMPILER_TYPE_GCC; then
        CCACHE_DEPEND=1 $CCACHE_COMPILE -std=c++20 "$CXX_MODULES_FLAG_GCC" -fdeps-format=p1689r5 -fdeps-file=foo.ddi -MD -MF foo.d -o foo.o -x c++ -c foo.cppm
        CCACHE_DEPEND=1 $CCACHE_COMPILE -std=c++20 "$CXX_MODULES_FLAG_GCC" -fdeps-format=p1689r5 -fdeps-file=bar.ddi -MD -MF bar.d -o bar.o -x c++ -c bar.cppm
        CCACHE_DEPEND=1 $CCACHE_COMPILE -std=c++20 "$CXX_MODULES_FLAG_GCC" -fdeps-format=p1689r5 -fdeps-file=main.ddi -MD -MF main.d -c main.cpp
    fi
